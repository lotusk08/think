...
{#if showMenu}
  <div class="menu-container" in:fly={{ y: 20, duration: 400, easing: quintOut }}>
	<nav id="menu" bind:this={menu} class:edit-mode={$show}>
	  <div class="menu-items">
		...
		<a href="#saveHTML" on:click|preventDefault={menuSaveAsHtml}>
		  <span class="menu-btn"><i class="fas fa-file-code"></i></span>
		</a>
		<a href="#saveSVG" on:click|preventDefault={menuSaveAsSvg}>
		  <span class="menu-btn"><i class="fas fa-file-arrow-down"></i></span>
		</a>
		<a href="#share" on:click|preventDefault={menuShare}>
		  <span class="menu-btn"><i class="fas fa-link"></i></span>
		</a>
		<a href="{$baseURL}" target="_blank" rel="noreferrer">
		  <span class="menu-btn"><i class="fas fa-question"></i></span>
		</a>
	  </div>
	  
	  <div class="eraser-container">
		{#if $show}
		  <a href="#reset" on:click|preventDefault={menuReset} transition:fade={{ duration: 200 }}>
			<span class="menu-btn"><i class="fas fa-eraser"></i></span>
		  </a>
		{/if}
	  </div>
	</nav>
	...
	{#if showNotification}
	  <div id="shareNotification" in:fly={{ y: 10, duration: 1000, easing: elasticOut }} out:fade>
